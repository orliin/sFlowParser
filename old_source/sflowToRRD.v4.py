...
##
def updateCounters() :
	#TODO: add global vlan graph for a switch??
	#TODO: add global traffic for a switch
	#TODO: add vlan traffic on a port
	if debug : 
	...
	if(not 'macs' in counters[agent]['ports'][inputPort]['vlans'][in_vlan]) : counters[agent]['ports'][inputPort]['vlans'][in_vlan]['macs'] = {}
	if(not srcMAC in counters[agent]['ports'][inputPort]['vlans'][in_vlan]['macs']) : counters[agent]['ports'][inputPort]['vlans'][in_vlan]['macs'][srcMAC] = {'input':0, 'output':0}
	if(not 'macs' in counters[agent]['ports'][outputPort]['vlans'][out_vlan]) : counters[agent]['ports'][outputPort]['vlans'][out_vlan]['macs'] = {}
	if(not dstMAC in counters[agent]['ports'][outputPort]['vlans'][out_vlan]['macs']) : counters[agent]['ports'][outputPort]['vlans'][out_vlan]['macs'][dstMAC] = {'input':0, 'output':0}

#	srcMacFlow = srcMAC+':'+dstMAC
#	dstMacFlow = dstMAC+':'+srcMAC
#	if(not 'macFlows' in counters[agent]['ports'][inputPort]['vlans'][in_vlan]) : counters[agent]['ports'][inputPort]['vlans'][in_vlan]['macFlows'] = {}
#	if(not srcMacFlow in counters[agent]['ports'][inputPort]['vlans'][in_vlan]['macFlows']) : counters[agent]['ports'][inputPort]['vlans'][in_vlan]['macFlows'][srcMacFlow] = {'input':0, 'output':0}
#	if(not 'macFlows' in counters[agent]['ports'][outputPort]['vlans'][out_vlan]) : counters[agent]['ports'][outputPort]['vlans'][out_vlan]['macFlows'] = {}
#	if(not dstMacFlow in counters[agent]['ports'][outputPort]['vlans'][out_vlan]['macFlows']) : counters[agent]['ports'][outputPort]['vlans'][out_vlan]['macFlows'][dstMacFlow] = {'input':0, 'output':0}
...
#	if(not flow in counters[agent]['vlans'][in_vlan]['flows']) : counters[agent]['vlans'][in_vlan]['flows'][flow] = {'traffic' : 0}
	...
	#in_vlan / out_vlan - all vlans in a port - sflow/{agent}/ports/{port}/vlans/{vlan}/{all port graphs for this vlan}
	...
	#srcMAC / dstMAC - all MAC addresses on a port in a vlan - sflow/{agent}/ports/{port}/vlans/{vlan}/macs/{mac}/{all port graphs for this mac}
	counters[agent]['ports'][inputPort]['vlans'][in_vlan]['macs'][srcMAC]['input'] += dataSize
	counters[agent]['ports'][outputPort]['vlans'][out_vlan]['macs'][dstMAC]['output'] += dataSize
	
	#srcMacFlow / dstMacFlow - all combinations input:output MAC on a port in a vlan - sflow/{agent}/ports/{port}/vlans/{vlan}/macFlows/{macFlow}/{all port graphs for this macFlow}
	counters[agent]['ports'][inputPort]['vlans'][in_vlan]['macFlows'][srcMacFlow]['input'] += dataSize
	counters[agent]['ports'][outputPort]['vlans'][out_vlan]['macFlows'][dstMacFlow]['output'] += dataSize
...
print "sflowtool -p 8343 | /usr/share/orliin/sflowToRRD.v4.py";
...
## Reading sample fields
		...
		if(sample['sampleType'] == 'FLOWSAMPLE') : 
			try:
				updateCounters()
